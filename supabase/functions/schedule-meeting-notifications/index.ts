...
Deno.serve(async (req) => {
  try {
    ...
    // Define reminders
    const reminderTimes = [
      { offsetMinutes: 24 * 60, type: 'reminder_24h', label: '24 hours' },
      { offsetMinutes: 60, type: 'reminder_1h', label: '1 hour' },
      { offsetMinutes: 15, type: 'reminder_15m', label: '15 minutes' }
    ];
    ...
    for (const user of [
      { id: user1Id, partnerName: user2Name },
      { id: user2Id, partnerName: user1Name }
    ]) {
      for (const reminder of reminderTimes) {
        ...
        if (notificationTimeUTC > DateTime.utc()) {
          ...
          console.log(`⏰ Notification ${reminder.type} for user ${user.id} is in the past, skipping...`);
          skippedCount++;
        }
      }
    }

    console.log(`📊 Summary for match ${matchingId}: ${successCount} scheduled, ${skippedCount} skipped, ${errorCount} errors`);
